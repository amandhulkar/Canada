...
function BusinessTemplate({ data = {} }) {
  ...
  return (
    <div className="font-sans">
      ...
      {/* NAVBAR */}
      <nav className="bg-white border-b border-gray-100 px-8 py-4 flex items-center justify-between sticky top-0 z-10">
        <span className="text-[#E91E8C] font-bold text-xl tracking-tight">{brand}</span>
        <div className="hidden md:flex items-center gap-8">
      ...
      {/* HERO */}
      <section id="tmpl-home" className="relative h-[480px] overflow-hidden">
        {heroImage
          ? <img src={heroImage} alt="Hero" className="w-full h-full object-cover" />
          : <div className="w-full h-full bg-gradient-to-br from-pink-400 to-indigo-500" />
        }
        <div className="absolute inset-0 bg-black/50 flex flex-col items-center justify-center text-center px-6">
          <h1 className="text-4xl md:text-6xl font-extrabold text-white leading-tight mb-4 max-w-3xl">{heroTitle}</h1>
          <p className="text-lg text-white/80 mb-8 max-w-xl">{heroSubtitle}</p>
          <div className="flex gap-4 flex-wrap justify-center">
            <button onClick={() => scrollTo("services")}
              className="px-8 py-3 rounded-full font-semibold text-white text-sm hover:opacity-90 transition"
              style={{ background: "#E91E8C" }}>Explore</button>
      ...
      {/* ABOUT */}
      <section id="tmpl-about" className="bg-[#F0F2F8] py-20 px-6">
        <div className="max-w-3xl mx-auto text-center">
          <p className="text-[#E91E8C] font-semibold text-sm uppercase tracking-widest mb-3">About Us</p>
          <h2 className="text-3xl font-extrabold text-[#1A1A2E] mb-6">Who We Are</h2>
          <p className="text-gray-500 text-lg leading-relaxed">{aboutText}</p>
        </div>
      </section>

      {/* SERVICES */}
      <section id="tmpl-services" className="bg-white py-16 px-6">
        <div className="max-w-6xl mx-auto text-center">
          <p className="text-[#E91E8C] font-semibold text-sm uppercase tracking-widest mb-2">What We Offer</p>
          <h2 className="text-2xl font-extrabold text-[#1A1A2E] mb-10">Our Services</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { icon: "⚡", title: service1Title, desc: service1Desc },
              { icon: "🎨", title: service2Title, desc: service2Desc },
              { icon: "💬", title: service3Title, desc: service3Desc },
            ].map((s) => (
              <div key={s.title} className="bg-[#F0F2F8] rounded-2xl p-6 text-center hover:shadow-md transition">
      ...
      {/* CTA */}
      <section className="py-16 px-6 text-center" style={{ background: "#E91E8C" }}>
        <h2 className="text-3xl font-extrabold text-white mb-4">{ctaHeading}</h2>
        <p className="text-white/80 mb-8 text-lg">{ctaSubtext}</p>
        <button onClick={() => scrollTo("contact")}
          className="bg-white font-bold px-10 py-3 rounded-full text-sm hover:scale-105 transition"
          style={{ color: "#E91E8C" }}>
          Contact Us Now
      ...
      {/* CONTACT */}
      <section id="tmpl-contact" className="bg-[#F0F2F8] py-16 px-6">
        <div className="max-w-xl mx-auto text-center">
          <p className="text-[#E91E8C] font-semibold text-sm uppercase tracking-widest mb-2">Get In Touch</p>
          <h2 className="text-3xl font-extrabold text-[#1A1A2E] mb-4">Contact Us</h2>
          {(contactEmail || contactPhone) && (
            <div className="flex justify-center gap-6 mb-6 flex-wrap">
              {contactEmail && <a href={`mailto:${contactEmail}`} className="text-[#E91E8C] text-sm font-semibold">📧 {contactEmail}</a>}
              {contactPhone && <a href={`tel:${contactPhone}`} className="text-[#E91E8C] text-sm font-semibold">📞 {contactPhone}</a>}
            </div>
          )}
          <div className="bg-white rounded-2xl p-8 shadow-sm text-left space-y-4">
            <input type="text" placeholder="Your Name" className="w-full border border-gray-200 rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-pink-300" />
            <input type="email" placeholder="Your Email" className="w-full border border-gray-200 rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-pink-300" />
            <textarea placeholder="Your Message" rows={4} className="w-full border border-gray-200 rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-pink-300" />
            <button className="w-full py-3 rounded-lg font-semibold text-white text-sm hover:opacity-90 transition" style={{ background: "#E91E8C" }}>
              Send Message
            </button>
          </div>
        </div>
