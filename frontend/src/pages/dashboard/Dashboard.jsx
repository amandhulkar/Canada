...
function AdminDashboard({ user }) {
  ...
  return (
    <div className="min-h-screen bg-stone-100 dark:bg-stone-900 flex">
      <Sidebar />
      <main className="flex-1 p-8">
        <h1 className="text-4xl font-bold text-amber-700 mb-6">Admin Dashboard</h1>
        {stats && (
          <div className="grid grid-cols-3 gap-4 mb-8">
            <div className="bg-white rounded-2xl shadow-sm p-6">
              <p className="text-gray-500 font-semibold">Total Users</p>
              <p className="text-4xl font-bold text-amber-700">{stats.totalUsers}</p>
            </div>
            <div className="bg-white rounded-2xl shadow-sm p-6">
              <p className="text-gray-500 font-semibold">Total Projects</p>
              <p className="text-4xl font-bold text-amber-700">{stats.totalProjects}</p>
            </div>
            <div className="bg-white rounded-2xl shadow-sm p-6">
              <p className="text-gray-500 font-semibold">Banned Users</p>
              <p className="text-4xl font-bold text-red-500">{stats.bannedUsers}</p>
            </div>
          </div>
        )}
            ...
            <tbody>
              {users.map((u) => (
                <tr key={u._id} className="border-b last:border-0">
                  ...
                  <td className="py-3">
                    <span className={`px-2 py-1 rounded-full text-xs font-bold ${u.role === "admin" ? "bg-amber-100 text-amber-700" : "bg-gray-100 text-gray-600"}`}>
                      {u.role}
                    </span>
                  </td>
                  <td className="py-3">
                    <span className={`px-2 py-1 rounded-full text-xs font-bold ${u.banned ? "bg-red-100 text-red-600" : "bg-green-100 text-green-600"}`}>
                      {u.banned ? "Banned" : "Active"}
                    </span>
                  </td>
                  <td className="py-3 flex gap-2">
                    <button onClick={() => banUser(u._id)} className={`px-3 py-1 rounded-lg text-xs font-semibold transition ${u.banned ? "bg-green-100 text-green-600 hover:bg-green-200" : "bg-amber-100 text-amber-700 hover:bg-amber-200"}`}>
                      {u.banned ? "Unban" : "Ban"}
                    </button>
                    <button onClick={() => deleteUser(u._id)} className="px-3 py-1 rounded-lg text-xs font-semibold bg-red-100 text-red-600 hover:bg-red-200 transition">
                      Delete
                    </button>
                  </td>
                </tr>
            ...
            <tbody>
              {projects.map((p) => (
                <tr key={p._id} className="border-b last:border-0">
                  ...
                  <td className="py-3">
                    <span className="px-2 py-1 rounded-full text-xs font-bold bg-amber-100 text-amber-700">{p.status}</span>
                  </td>
                  <td className="py-3 text-gray-500">{p.user?.fullName || "N/A"}</td>
